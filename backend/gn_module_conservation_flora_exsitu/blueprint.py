...
from apptax.taxonomie.models import BibAttributs, CorTaxonAttribut, Taxref
...
from sqlalchemy import String, case, cast, exists
...
def get_material_type_code(material):
    ...
    return (
        ...
        .filter(TNomenclatures.id_nomenclature == material.id_material_type)
        .scalar()
    )
...
def material_has_taxon(material):
    ...
    return CorMaterialTaxon.query.filter_by(id_material=material.id_material).first() is not None


def material_can_have_seed_description(material):
    return get_material_type_code(material) == SEED_MATERIAL_CODE and material_has_taxon(material)
...
def group_geometries(results):
    ...
    grouped_results = [
        {
            "geom": geom,
            "harvest_ids": list(set(harvest_ids)),
        }  # Enlève les doublons dans les IDs
...
def get_location_types():
    try:
        ...
        return jsonify({"COMMUNE_ID": commune_id, "DEPARTEMENT_ID": departement_id})
    ...
    features = [
        Feature(
            ...
            properties={
                "harvest_ids": result[
                    "harvest_ids"
                ]  # Liste des IDs des récoltes ayant cette géométrie
            },
        )
        for result in grouped_results
    ]

    return {"items": FeatureCollection(features)}, 200
...
def get_harvest_by_id(harvest_id):
    ...
    geographical_location = (
        ...
        .filter(TNomenclatures.id_nomenclature == harvest.id_geographical_precision)
        .first()
    )
    geographical_location = {
    ...
    materials_exist = (
        db.session.query(TMaterial).filter(TMaterial.id_harvest == harvest.id_harvest).count() > 0
    )
    geom_geojson = None
    if harvest.geom:
        geom_wgs84 = (
            db.session.query(func.ST_AsGeoJSON(func.ST_Transform(THarvest.geom, 4326)))
            .filter(THarvest.id_harvest == harvest.id_harvest)
            .first()
        )
        if geom_wgs84:
...
def get_harvest_details(harvest_id):
    try:
        ...
        precision_value = (
            f"{harvest.precision} m" if harvest.precision is not None else "Non renseignée"
        )
        ...
        if geographical_precision_code == "com":
            location_name = f"(C) {area_name}" if area_name else "Non renseignée"

            id_area_type = "Commune"

        elif geographical_precision_code == "dept":
            location_name = f"(D) {area_name}" if area_name else "Non renseignée"
        ...
        observer_name = (
            f"{main_observer.nom_role} {main_observer.prenom_role}" if main_observer else "N/A"
        )
...
def create_material(id_harvest):
    ...
    if taxons:
        for cd_nom in taxons:
            existing_entry = CorMaterialTaxon.query.filter_by(
                id_material=material.id_material, cd_nom=cd_nom
            ).first()
    ...
    return {"message": "Matériel ajouté", "material": material.to_dic()}, 201
...
def update_material(id_harvest, id_material):
    ...
    has_seed_description = (
        TMaterielSeed.query.filter_by(id_material=id_material).first() is not None
    )
    ...
    if has_seed_description:
        ...
        if requested_material_type is not None:
            requested_material_type_code = (
                db.session.query(TNomenclatures.cd_nomenclature)
                .filter(TNomenclatures.id_nomenclature == requested_material_type)
                .scalar()
            )
        ...
        if requested_taxons is not None and len(requested_taxons) == 0:
            return {
                "error": "Modification impossible",
                "message": (
                    "Ce matériel récolté possède une fiche Semence. "
...
def delete_material(id_material):
    ...
    storage_count = TStorage.query.filter_by(id_material=id_material).count()

    germination_test_count = (
        db.session.query(TTest)
        .join(TNomenclatures, TTest.id_test_type == TNomenclatures.id_nomenclature)
        .filter(TTest.id_material == id_material, TNomenclatures.cd_nomenclature == "ger")
        .count()
    )
    ...
    viability_test_count = (
        db.session.query(TTest)
        .join(TNomenclatures, TTest.id_test_type == TNomenclatures.id_nomenclature)
        .filter(TTest.id_material == id_material, TNomenclatures.cd_nomenclature == "via")
        .count()
    )

    sowing_count = TSowing.query.filter_by(id_material=id_material).count()

    culture_count = TCulture.query.filter_by(id_material=id_material).count()
...
def get_materials_by_harvest(id_harvest):
    ...
    return jsonify(
        [{"id_material": m.id_material, "code_material": m.code_material} for m in materials]
    )
...
def get_materials(id_harvest):
    try:
        ...
        materials = (
            TMaterial.query.filter_by(id_harvest=id_harvest)
        ...
        for material in materials:
            ...
            for taxon in taxons:
                ...
                if taxon_data:
                    taxon_list.append({"cd_nom": taxon.cd_nom, "nom_valide": taxon_data.nom_valide})

            nomenclature_material = (
                db.session.query(TNomenclatures.label_default, TNomenclatures.cd_nomenclature)
                .filter_by(id_nomenclature=material.id_material_type)
                .first()
            )
            harvest_material_label = (
                nomenclature_material.label_default if nomenclature_material else None
            )
            harvest_material_code = (
                nomenclature_material.cd_nomenclature if nomenclature_material else None
            )
            ...
            if material.id_action:
                culture_context = (
                    ...
                    .join(TAction, TAction.id_culture == TCulture.id_culture)
                    .filter(TAction.id_action == material.id_action)
                    .first()
                )

                if culture_context:
                    source_material = TMaterial.query.get(culture_context.id_material)

                    if source_material:
                        code_cultural_bank_material = (
                            f"{source_material.code_material} - {culture_context.code_culture}"
                        )
            ...
            material_dict["sowing_count"] = TSowing.query.filter_by(
                id_material=material.id_material
            ).count()

            material_dict["culture_count"] = TCulture.query.filter_by(
                id_material=material.id_material
            ).count()
...
def add_taxon_to_material(id_material):
    try:
        ...
        existing_entry = CorMaterialTaxon.query.filter_by(
            id_material=id_material, cd_nom=cd_nom
        ).first()
        if existing_entry:
...
def delete_material_taxon(id_material, cd_nom):
    try:
        ...
        if has_seed_description:
            taxon_count = CorMaterialTaxon.query.filter_by(id_material=id_material).count()
...
def search_code_material():
    ...
    results = (
        db.session.query(TMaterial.code_material)
        .filter(TMaterial.code_material.ilike(f"%{search_term}%"))
        .limit(10)
        .all()
    )
...
def export_harvests():
    ...
    harvests = (
        db.session.query(
            ...
            func.string_agg(func.distinct(User.nom_role + " " + User.prenom_role), ", ").label(
                "observers_list"
            ),
            ...
            case([(THarvest.id_area_type == commune_id, LAreas.area_name)], else_=None).label(
                "commune"
            ),
            case(
                [(THarvest.id_area_type == departement_id, LAreas.area_name)],
                else_=None,
            ).label("departement"),
            ...
            case(
                ...
                else_="Non",
            ).label("risque_hybridation"),
            TMaterial.remarks.label("protocoles_astuces"),
            func.string_agg(func.distinct(cast(CorMaterialTaxon.cd_nom, String)), ", ").label(
                "cd_noms"
            ),
        ...
        .outerjoin(
            main_observer_subquery,
            main_observer_subquery.c.id_harvest == THarvest.id_harvest,
        )
        .outerjoin(TMaterial, THarvest.id_harvest == TMaterial.id_harvest)
        .outerjoin(
            FootCountingClass,
            TMaterial.id_foot_counting_class == FootCountingClass.id_nomenclature,
        )
        .outerjoin(
            HarvestMaterial,
            TMaterial.id_material_type == HarvestMaterial.id_nomenclature,
        )
        ...
        csv_writer.writerow(
            {
                ...
                "Coordonnées": f"{h.latitude:.6f}, {h.longitude:.6f}"
...
def add_multiple_media_to_seed(id_seed):
    ...
    if code_nomenclature == "2":
        for file in files:
            ...
            media_path = str(
                media_path_full.relative_to(current_app.config["MEDIA_FOLDER"] + "/attachments")
            )
    ...
    return {
        ...
        "ids": [m.id_media for m in created_media_ids],
    }, 201
...
def update_media_for_seed(id_seed):
    ...
    existing_media = TMedias.query.filter_by(uuid_attached_row=seed.unique_id_seed).all()

    for media in existing_media:
        if media.media_path:
            try:
                old_path = (
                    Path(current_app.config["MEDIA_FOLDER"]) / "attachments" / media.media_path
                )
                if old_path.exists():
                    old_path.unlink()
            except Exception as e:
...
def get_seed_of_material(id_material):
    ...
    medias = (
        TMedias.query.filter_by(uuid_attached_row=seed.unique_id_seed)
...
def get_full_seed_info(id_seed):
    seed = db.session.query(TMaterielSeed).filter(TMaterielSeed.id_seed == id_seed).first()
    ...
    if link:
        cd_nom = link.cd_nom
        cd_ref = db.session.query(Taxref.cd_ref).filter(Taxref.cd_nom == cd_nom).scalar()
        if cd_ref:
            query = (
                db.session.query(
    ...
    media_entries = (
        ...
        .filter(TMedias.uuid_attached_row == str(seed.unique_id_seed))
        .all()
    )
    ...
    media_taxhub = db.session.query(TMediasTaxHub).filter(TMediasTaxHub.cd_ref == cd_ref).all()
...
def update_action(id_material, id_storage):
    ...
    try:
        ...
        return {
            ...
            "id_storage": updated_action.id_storage,
        }, 200
    except ValueError as ve:
        return jsonify({"error": str(ve)}), 400
    except Exception as e:
...
def get_actions_by_place(id_material):
    ...
    try:
        ...
        return {"items": results, "total": total, "page": page, "limit": limit}, 200
    except ValueError as ve:
        print(str(ve))
        return jsonify({"error": str(ve)}), 400
    except Exception as e:
...
def delete_action(id_material, id_storage):
    ...
    try:
        ...
        if action.id_storage_action == code_stock_init:
            ...
            ids_outputs = [
                action_repo.get_id_nomenclature("CFE_STORAGE_ACTION", c) for c in codes_outputs
            ]
...
@blueprint.route("/materials/<int:id_material>/cultures", methods=["POST"])
@permissions.check_cruved_scope("C", module_code=MODULE_CODE)
@json_resp
def create_culture(id_material):
    try:
        ...
        culture = repo.create(id_material, data)
        ...
        current_app.logger.exception("create_culture failed")

        return {"error": "Erreur interne du serveur"}, 500


@blueprint.route("/materials/<int:id_material>/cultures", methods=["GET"])
@permissions.check_cruved_scope("R", module_code=MODULE_CODE)
@json_resp
def list_cultures_by_material(id_material):
    ...
    id_sowing = request.args.get("id_sowing", type=int)
    ...
    if source_type == "material":
        return repo.get_direct_by_material(id_material), 200
    ...
    if source_type == "sowing":
        if not id_sowing:
            return {"error": ("L'identifiant du semis est obligatoire")}, 400

        try:
            return repo.get_all_by_sowing(id_material, id_sowing), 200
    ...
    if source_type == "test":
        if not id_test:
            return {"error": ("L'identifiant du test est obligatoire")}, 400

        try:
            return repo.get_all_by_test(id_material, id_test), 200
    ...
    # -----------------------------------------
    return repo.get_all_by_material(id_material), 200


@blueprint.route("/cultures/<int:id_culture>", methods=["GET"])
@permissions.check_cruved_scope("R", module_code=MODULE_CODE)
@json_resp
def get_culture(id_culture):
    repo = CultureRepository()

    culture = repo.get_with_labels_by_id(id_culture)
...
@blueprint.route("/cultures/<int:id_culture>/actions/transplantation", methods=["POST"])
@permissions.check_cruved_scope("C", module_code=MODULE_CODE)
@json_resp
def create_culture_transplantation(id_culture):
    try:
        culture = CultureRepository().get_by_id(id_culture)
        ...
        transplantation_data = data.get("transplantation") or {}

        result = CultureActionTransplantationRepository().create_with_action(
            id_culture=id_culture,
            action_data=action_data,
            transplantation_data=(transplantation_data),
            meta_create_by=(g.current_user.id_role),
        )

        return {
            "message": ("Action de transplantation créée avec succès"),
            **result,
        }, 201
        ...
        current_app.logger.exception("create_culture_transplantation failed")

        return {"error": "Erreur interne du serveur"}, 500


@blueprint.route("/actions/<int:id_action>/transplantation", methods=["GET"])
@permissions.check_cruved_scope("R", module_code=MODULE_CODE)
@json_resp
def get_culture_transplantation(id_action):
    transplantation = CultureActionTransplantationRepository().get_by_action(id_action)

    if not transplantation:
        return {"error": ("Action de transplantation non trouvée")}, 404

    return transplantation, 200


@blueprint.route("/actions/<int:id_action>/transplantation", methods=["PUT"])
@permissions.check_cruved_scope("U", module_code=MODULE_CODE)
@json_resp
def update_culture_transplantation(id_action):
    try:
        data = request.get_json(silent=True) or {}
        ...
        result = CultureActionTransplantationRepository().update_with_action(
            id_action=id_action,
            action_data=action_data,
            transplantation_data=(transplantation_data),
            meta_update_by=(g.current_user.id_role),
        )
        ...
        return {
            "message": ("Action de transplantation mise à jour avec succès"),
        ...
        current_app.logger.exception(
            f"update_culture_transplantation failed for action {id_action}"
        )

        return {"error": "Erreur interne du serveur"}, 500


@blueprint.route("/cultures/<int:id_culture>/actions/observation", methods=["POST"])
@permissions.check_cruved_scope("C", module_code=MODULE_CODE)
@json_resp
def create_culture_observation(id_culture):
    try:
        culture = CultureRepository().get_by_id(id_culture)
        ...
        result = CultureActionObservationRepository().create_with_action(
            id_culture=id_culture,
            action_data=action_data,
        ...
        return {"message": ("Action d'observation créée avec succès"), **result}, 201

    except ValueError as error:
        return {"error": str(error)}, 400

    except Exception:
        current_app.logger.exception("create_culture_observation failed")

        return {"error": "Erreur interne du serveur"}, 500


@blueprint.route("/actions/<int:id_action>/observation", methods=["GET"])
@permissions.check_cruved_scope("R", module_code=MODULE_CODE)
@json_resp
def get_culture_observation(id_action):
    observation = CultureActionObservationRepository().get_by_action(id_action)

    if not observation:
        return {"error": ("Action d'observation non trouvée")}, 404

    return observation, 200


@blueprint.route("/actions/<int:id_action>/observation", methods=["PUT"])
@permissions.check_cruved_scope("U", module_code=MODULE_CODE)
@json_resp
def update_culture_observation(id_action):
    try:
        data = request.get_json(silent=True) or {}
        ...
        result = CultureActionObservationRepository().update_with_action(
            id_action=id_action,
            action_data=action_data,
            observation_data=(observation_data),
            meta_update_by=(g.current_user.id_role),
        ...
        return {
            "message": ("Action d'observation mise à jour avec succès"),
            "observation": result,
        }, 200
        ...
        current_app.logger.exception(f"update_culture_observation failed for action {id_action}")

        return {"error": "Erreur interne du serveur"}, 500


@blueprint.route("/cultures/<int:id_culture>/actions/treatment", methods=["POST"])
@permissions.check_cruved_scope("C", module_code=MODULE_CODE)
@json_resp
def create_culture_treatment(id_culture):
    try:
        culture = CultureRepository().get_by_id(id_culture)
        ...
        result = CultureActionTreatmentRepository().create_with_action(
            id_culture=id_culture,
            action_data=action_data,
        ...
        return {
            "message": ("Action de traitement de culture créée avec succès"),
            **result,
        }, 201
        ...
        current_app.logger.exception("create_culture_treatment failed")

        return {"error": "Erreur interne du serveur"}, 500


@blueprint.route("/actions/<int:id_action>/treatment", methods=["GET"])
@permissions.check_cruved_scope("R", module_code=MODULE_CODE)
@json_resp
def get_culture_treatment(id_action):
    treatment = CultureActionTreatmentRepository().get_by_action(id_action)

    if not treatment:
        return {"error": ("Action de traitement de culture non trouvée")}, 404

    return treatment, 200


@blueprint.route("/actions/<int:id_action>/treatment", methods=["PUT"])
@permissions.check_cruved_scope("U", module_code=MODULE_CODE)
@json_resp
def update_culture_treatment(id_action):
    try:
        data = request.get_json(silent=True) or {}
        ...
        result = CultureActionTreatmentRepository().update_with_action(
            id_action=id_action,
            action_data=action_data,
            treatment_data=(treatment_data),
            meta_update_by=(g.current_user.id_role),
        ...
        return {
            "message": ("Action de traitement de culture mise à jour avec succès"),
            "treatment": result,
        }, 200
        ...
        current_app.logger.exception(f"update_culture_treatment failed for action {id_action}")

        return {"error": "Erreur interne du serveur"}, 500


@blueprint.route("/cultures/<int:id_culture>/actions/sampling", methods=["POST"])
@permissions.check_cruved_scope("C", module_code=MODULE_CODE)
@json_resp
def create_culture_sampling(id_culture):
    try:
        culture = CultureRepository().get_by_id(id_culture)
        ...
        result = CultureActionSamplingRepository().create_with_action(
            id_culture=id_culture,
            action_data=action_data,
        ...
        return {"message": ("Action de prélèvement créée avec succès"), **result}, 201

    except ValueError as error:
        return {"error": str(error)}, 400

    except Exception:
        current_app.logger.exception("create_culture_sampling failed")

        return {"error": "Erreur interne du serveur"}, 500


@blueprint.route("/actions/<int:id_action>/sampling", methods=["GET"])
@permissions.check_cruved_scope("R", module_code=MODULE_CODE)
@json_resp
def get_culture_sampling(id_action):
    sampling = CultureActionSamplingRepository().get_by_action(id_action)

    if not sampling:
        return {"error": ("Action de prélèvement non trouvée")}, 404

    return sampling, 200


@blueprint.route("/actions/<int:id_action>/sampling", methods=["PUT"])
@permissions.check_cruved_scope("U", module_code=MODULE_CODE)
@json_resp
def update_culture_sampling(id_action):
    try:
        data = request.get_json(silent=True) or {}
        ...
        result = CultureActionSamplingRepository().update_with_action(
            id_action=id_action,
            action_data=action_data,
            sampling_data=(sampling_data),
            meta_update_by=(g.current_user.id_role),
        ...
        return {
            "message": ("Action de prélèvement mise à jour avec succès"),
            "sampling": result,
        }, 200
        ...
        current_app.logger.exception(f"update_culture_sampling failed for action {id_action}")

        return {"error": "Erreur interne du serveur"}, 500


@blueprint.route("/materials/<int:id_material>/cultures/<int:id_culture>", methods=["PUT"])
@permissions.check_cruved_scope("U", module_code=MODULE_CODE)
@json_resp
def update_culture(id_material, id_culture):
    try:
        data = request.get_json(silent=True) or {}
        ...
        culture = repo.update(id_material, id_culture, data)
        ...
        current_app.logger.exception("update_culture failed")

        return {"error": "Erreur interne du serveur"}, 500


@blueprint.route("/materials/<int:id_material>/cultures/<int:id_culture>", methods=["DELETE"])
@permissions.check_cruved_scope("D", module_code=MODULE_CODE)
@json_resp
def delete_culture(id_material, id_culture):
    action_count = TAction.query.filter_by(id_culture=id_culture).count()

    if action_count > 0:
        return {
            "error": "Suppression impossible",
            "message": "Cette culture contient des actions liées.",
            "action_count": action_count,
        }, 409
    ...
    return {"message": "Culture supprimée avec succès"}, 200
...
def get_actions_by_id_sowing(id_sowing):
    try:
        ...
        current_app.logger.error(
            f"Erreur lors du chargement des actions pour le semis {id_sowing}: {e}"
        )
        return jsonify({"error": "Erreur interne du serveur"}), 500
...
def get_test_by_material(id_material):
    ...
    return jsonify([{"id_test": m.id_test, "code": m.code} for m in test])
...
def get_all_tests_by_material(id_material):
    ...
    tests = (
        db.session.query(TTest, TestType.cd_nomenclature.label("test_type_code"))
...
def update_test(id_material, id_test):
    ...
    try:
        ...
        return jsonify(
            {"message": "Test mis à jour avec succès", "id_test": updated_test.id_test}
        ), 200
    except ValueError as ve:
        return jsonify({"error": str(ve)}), 400
    except Exception as e:
...
def get_actions_code_autocomplete(id_test):
    ...
    return jsonify([{"id_action": action.id_action, "code": action.code} for action in actions])
...
def get_actions_by_id_test(id_test):
    try:
        ...
        current_app.logger.error(
            f"Erreur lors du chargement des actions pour le test {id_test}: {e}"
        )
        return jsonify({"error": "Erreur interne du serveur"}), 500


@blueprint.route("/cultures/<int:id_culture>/actions", methods=["GET"])
@permissions.check_cruved_scope("R", module_code=MODULE_CODE)
@json_resp
def get_actions_by_id_culture(id_culture):
    try:
        culture = CultureRepository().get_by_id(id_culture)
        ...
        actions = ActionRepository().get_actions_by_id_culture(id_culture)

        return actions, 200

    except Exception:
        current_app.logger.exception(f"get_actions_by_id_culture failed for Culture {id_culture}")
...
def delete_actio(id_action):
    try:
        ...
        linked_material = TMaterial.query.filter_by(id_action=id_action).first()
        ...
        if linked_material:
            ...
            if (
                ...
                return {
                    "error": "Suppression impossible",
                    "message": (
                        "Le matériel récolté lié à cette action contient des données liées."
                    ),
                    "id_material": id_material,
                    "code_material": linked_material.code_material,
                    "has_seed_description": has_seed_description,
                    "storage_count": storage_count,
                    "germination_test_count": germination_test_count,
                    "viability_test_count": viability_test_count,
            ...
            deleted = HarvestMaterialRepository().delete(id_material)

            if not deleted:
                return {"message": "Matériel récolté non trouvé"}, 404

            return {"message": ("Action et matériel récolté supprimés avec succès")}, 200

        if action.id_culture:
            initial_action = CultureRepository.get_initial_action(action.id_culture)

            is_initial_action = (
                initial_action is not None and initial_action.id_action == action.id_action
            )

            if is_initial_action and CultureRepository.has_initial_transplantation(
                action.id_culture
            ):
                action_count = TAction.query.filter_by(id_culture=action.id_culture).count()

                if action_count > 1:
                    culture = CultureRepository().get_by_id(action.id_culture)
                    ...
                    return {
                        ...
                        "action_count": action_count,
                        "remaining_action_count": (action_count - 1),
                        "code_culture": (culture.code_culture if culture else None),
                    }, 409
...
@blueprint.route(
    "/materials/<int:id_material>/tests/<int:id_test>/actions/<int:id_action>/replicates",
    methods=["PUT"],
)
@permissions.check_cruved_scope("U", module_code=MODULE_CODE)
...
def get_germination_percent(id_test):
    ...
    actions = TAction.query.filter_by(
        id_test=id_test, id_action_type=svr_nomenclature.id_nomenclature
    ).all()
    ...
    total_germinated = (
        db.session.query(func.sum(TActionReplicate.count_germinated))
        .filter(TActionReplicate.id_action.in_(action_ids))
        .scalar()
        or 0
    )
